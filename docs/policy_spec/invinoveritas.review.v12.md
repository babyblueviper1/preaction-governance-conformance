# `invinoveritas.review.v12` — the rubric a `policy_version` label actually names

Supersedes `invinoveritas.review.v10` (same repo, same path pattern) — v10 introduced the
recomputable `policy_commitment` construction described below; v11 (2026-08-15) changed only the
`source_class=independent_mediator` binding (see §3) and reused v10's rubric bytes unchanged, so it
never needed its own published doc. v12 (2026-08-25) is the first version since v10 to change the
rubric TEXT itself — a new decision-boundary rule on disclosed limitations (see §1, §3) — so a new
file is required rather than a byte-identical carry-forward.
...
stranger-auditable; a signature only proves WHO signed, not WHAT logic ran. Before v10, this
...
should pass ARE deterministically pinnable, and have been pinned since v10.

**What `policy_commitment` binds**, computed as:

```
policy_commitment = sha256(JCS({
  "policy_version": "invinoveritas.review.v12",
...
hold `invinoveritas.review.v12` to, recomputable by a stranger, not asserted on trust. That is the
...
- A disclosed limitation lowers severity when it bounds the claim, not when it contradicts it.
  Bounding: the author states what the claim does NOT establish, and the remaining claim is then
  true at that narrower scope — that is honest scope, not a defect; do not treat hedge language
  itself as a finding. Contradicting: the disclosure shows the remaining claim is still false (the
  named limitation falsifies the assertion, or a defect remains undisclosed). Pre-disclosing a real
  flaw does not launder it past review — score that contradiction at the same severity as if the
  defect were unnamed. Test: after applying the limitation as a scope restriction, is the remaining
  claim true? If yes, lower severity. If no, do not. (v12, see §3.)
...
**v10** — this is the fix: `policy_commitment` (the hash construction at the top of this file)
added to the preimage, so `policy_version` stops being a bare trusted label and becomes a
recomputable pointer to the exact rubric text and conformance-suite commit it claims to mean. No
rubric or field-list change beyond adding a doc and binding its hash — the review criteria in
§1/§2 were the same criteria v9 already ran under; this version exists specifically to make that
claim checkable rather than asserted. (Published as `invinoveritas.review.v10.md` at commit
`62d32e519c39ba060950140ee7ceb5327371a0fe` — that file's bytes are unchanged and remain the
historical record for any `decision_ref` still recomputing under v10/v11.)

**v11** (2026-08-15, observation-conditions-note / Pavlo) — the authority relation for
`source_class=independent_mediator` was a live "now" read of a gitignored registry keyed by raw
Bearer tokens. Three distinct gaps: (1) `verified_at` existed on every proof but was not in the
preimage — tamper-strippable; (2) `mediator_evidence.registry_snapshot_sha256` was informational
only — also strip-able; (3) the registry itself had no external version history, and could not be
git-tracked as-is because its top-level keys WERE live API tokens. Fix: public registry keyed by
`mediator_id` (git-tracked), secret api_key→id map stays gitignored; bind `verified_at`,
`registry_as_of` (verified_at when elevation held, else null), and `registry_snapshot_sha256` into
the preimage. Review criteria / rubric bytes unchanged from v10 (same `RUBRIC_SHA256`/doc path) —
this bump was the binding + as-of scope, not a new judgment rubric, which is why it never needed
its own published file until now.

**v12 (this document, 2026-08-25)** — a real, live gap found across three rounds of correspondence
with Dipankar Sarkar (Substack): the rubric did not distinguish a disclosed limitation that BOUNDS
a claim (honest scope, not a defect) from one that CONTRADICTS it (the claim is still false, or an
undisclosed defect remains) — real testing (paired hedged/confident replications on 3 independent
artifacts, plus a laundering-attack control) confirmed the effect was real, not a hedge-language
artifact: the reviewer was reasoning about the underlying mechanism, and an undifferentiated
severity penalty on ANY disclosed limitation both punished honest scoping and, if inverted
naively, would have let a pre-disclosed real flaw launder past review at a lower severity. Added
the decision-boundary rule in §1 above, verified against all 3 historical pairs plus the new
laundering control before publishing (all 4 directions correct: bounded claims discounted,
contradictions/undisclosed defects still flag at full severity, pre-disclosure does not launder).
No field-list change — this is a rubric-TEXT change, the first since v10, hence the new
`rubric_sha256`/doc path rather than a byte-identical carry-forward like v11.
