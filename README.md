...
| [`ledger-chain-continuity`](examples/ledger-chain-continuity) | issue #3's shape against our OWN real production data: 4 real consecutive `/ledger` entries (245-248), recomputes the live hash-chain (`services/ledger_chain.py`) offline, and an omission negative control shows a per-entry-only check would miss a skipped entry while the sequence-level `prev_link` check catches it |
...
| [`schnorr-tamper-matrix`](examples/schnorr-tamper-matrix) | ethereum-magicians t/29275 (helmymekaoui-web) — his exact 4-vector tamper matrix (content/timestamp/sig-bit/pubkey-swap) run against a real `/ledger` verdict; closes the schnorr/NIP-01 leg honestly and names the ECDSA/secp256k1 cross-scheme adapter as separate, unbuilt work rather than papering over it |
| [`verdict-ref-v1-adapter`](examples/verdict-ref-v1-adapter) | microsoft/autogen#7353 (giskard09) — recomputes all 4 of their published verdict-ref-v1 vectors byte-identical under our own hash function, then emits a NEW verdict_object programmatically from a real live `/review` call (not a hand-typed one-off) |
